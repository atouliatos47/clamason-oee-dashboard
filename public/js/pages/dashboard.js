...
function renderDashboard() {
    ...
    <div style="display:grid;grid-template-columns:190px 1fr 210px;gap:12px;margin-bottom:12px;">
        ...
        <div class="card" style="text-align:center;padding:16px 10px 12px;">
            <div style="font-size:11px;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px;">Fleet OEE</div>
            <canvas id="gaugeHome" width="160" height="100" style="width:100%;max-width:160px;display:block;margin:0 auto;"></canvas>
            <div style="font-size:26px;font-weight:700;color:${avgOEECol};margin-top:4px;line-height:1;">${fmt1(avgOEE)}%</div>
            <div style="font-size:10px;font-weight:600;color:#aaa;text-transform:uppercase;letter-spacing:.5px;margin-top:2px;">OEE</div>
            <div style="display:flex;justify-content:space-between;margin-top:10px;padding-top:8px;border-top:1px solid #f0f0f0;">
                <div style="font-size:10px;color:#888;">Target <strong style="color:#243547;">${state.wcTarget}%</strong></div>
                <div style="font-size:10px;color:#888;">${wk || '—'}</div>
            </div>
        </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
        ...
        <div class="card" style="border-top:4px solid #243547;cursor:pointer;padding:14px 16px;"
            ...
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:10px;">
                ${[
            ['OEE', fmt1(avgOEE) + '%', avgOEECol],
            ['MTBF', equipMTBF > 0 ? equipMTBF + 'h' : '—', mtbfCol],
            ['Avail', fmt1(avgAvail) + '%', availCol],
        ].map(([l, v, c]) => `
        ...
        <div class="card" style="border-top:4px solid #7b5ea7;cursor:pointer;padding:14px 16px;"
            ...
            <div style="font-size:11px;color:#7b5ea7;font-weight:700;">Due Date · Tool jobs · KPIs →</div>
        </div>
    </div>`;
    ...
    const top5 = [...maintCurrent].filter(m => +m.downtime_hrs >= 0.5)
