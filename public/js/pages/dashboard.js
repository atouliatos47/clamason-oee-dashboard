...
function renderScheduleChart() {
    ...
    const W = 560, H = 180, padL = 44, padB = 28, padT = 10, padR = 10;
    const chartH = H - padT - padB, chartW = W - padL - padR;
    const bw = Math.min(40, chartW / weeks.length - 6);
    ...
    weekData.forEach((d, i) => {
        ...
        const runH  = (d.run  / yMax) * chartH;
        const unplH = (d.unpl / yMax) * chartH;
        ...
        if (d.run > 0) bars += `<text x="${x.toFixed(1)}" y="${(baseY - runH - 3).toFixed(1)}" text-anchor="middle" font-size="8" fill="#243547" font-weight="700">${d.run}h</text>`;
    ...
    if (adh_pts.length > 1) {
        ...
        adh_pts.forEach(p => {
            adhLine += `<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="3" fill="#243547" stroke="#fff" stroke-width="1.5"/>`;
            adhLine += `<text x="${p.x.toFixed(1)}" y="${(p.y-6).toFixed(1)}" text-anchor="middle" font-size="8" fill="#243547" font-weight="700">${p.adh}%</text>`;
        });
    }
    ...
    yTicks.forEach(v => {
        ...
        yLabels += `<text x="${padL-4}" y="${(y+3).toFixed(1)}" text-anchor="end" font-size="8" fill="#bbb">${v}h</text>`;
    ...
    <div style="font-size:11px;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">
        ...
        <span style="float:right;display:flex;align-items:center;gap:12px;font-size:11px;font-weight:700;color:#243547">
            <span style="display:flex;align-items:center;gap:4px"><span style="width:16px;height:10px;background:#95C11F;border-radius:2px;display:inline-block"></span>Run Hours</span>
            <span style="display:flex;align-items:center;gap:4px"><span style="width:16px;height:10px;background:#c0392b;border-radius:2px;display:inline-block"></span>Unplanned Down</span>
